...
contract GCVesting is Ownable {
    ...
    /**
     *  @dev The GC token cannot be transfered out of the contract
     */
    error GreenTokenCannotBeTransfered();
    ...
    function claim() external onlyOwner {
        ...
        if (block.timestamp >= VESTING_END) {
            amount = IERC20(GCA).balanceOf(address(this));
        } else {
            amount =
        ...
        IERC20(GCA).safeTransfer(RECIPIENT, amount);
    }

    function recoverToken(address token_) external onlyOwner {
        if (token_ == GCA) revert GreenTokenCannotBeTransfered();
